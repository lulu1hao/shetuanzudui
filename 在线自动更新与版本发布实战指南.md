...
- `lulu_3.4.0_x64-setup.exe`（**更新包本体**）
- `lulu_3.4.0_x64-setup.exe.sig`（**数字签名文件**）
...
1. 运行一键模板生成命令：
   ...
   ```
2. 脚本会读取当前版本的 `.exe.sig` 并自动填入签名和 GitHub Release 下载地址。打开根目录下生成的 `latest.template.json`，修改更新说明后复制为 `updates/latest.json`：
   ```json
   {
     "version": "3.4.0",
     ...
     "platforms": {
       "windows-x86_64": {
        "signature": "【脚本自动读取 lulu_3.4.0_x64-setup.exe.sig 的内容】",
        "url": "https://github.com/lulu1hao/shetuanzudui/releases/download/v3.4.0/lulu_3.4.0_x64-setup.exe"
       }
     }
   }
   ```
3. 发布前必须执行一致性检查：
   ...
   ```
   该命令会检查 `package.json`、`Cargo.toml`、`tauri.conf.json`、下载地址版本和数字签名所属文件，防止旧签名误用于新安装包。
...
3. 将 `lulu_3.4.0_x64-setup.exe` 上传，并将校验通过的 `updates/latest.json` 提交到 `version2` 分支后点击 **Publish release**。
